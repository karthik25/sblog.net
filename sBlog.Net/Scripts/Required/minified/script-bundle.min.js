...
function savePostOrPage(){if(!$('.privateChkBox').is(':checked')){$('.privateChkBox').trigger('click');}
var rUrl=($('#AjaxSaved').val().toLowerCase()=='true')?'edit':'add';var formData=$('form').serializeArray();$.ajax({type:'POST',url:siteRoot+'admin/post/'+rUrl,data:formData,dataType:'json',success:function(data){if(data.IsValid){$('#AjaxSaved').val(true);$('#Post_PostID').val(data.PostId);var msg='Saved a draft of your post at <b>'+getCurrentDateTime()+'</b>';$('#updateAjaxStatus').html(msg).show();}}});}
